...
export default function Home() {
  return <Shell>
    <section className="hero"><p className="eyebrow">Skyline by HC · Singapore</p><h1>Developer decisions shape construction outcomes <em>long before work begins on site.</em></h1><div className="hero-bottom"><div><p>I’m Lim Hwee Chim, a Singapore-based developer-side property practitioner. I write about the decisions, interfaces and human judgement behind the built environment.</p><p className="hero-mission">Hard-won project lessons, made useful for the next decision.</p></div><div className="hero-actions"><Link className="button primary" href="/articles">Start with selected articles <span>→</span></Link><Link className="text-link" href="/about">About Lim Hwee Chim <span>→</span></Link></div></div></section>
    <section className="feature"><div className="feature-label"><span>Featured essay</span><span>01</span></div><div className="feature-copy"><p className="eyebrow">{articles[0].topic} · {articles[0].read} read</p><h2><Link href={`/articles/${articles[0].slug}`}>{articles[0].title}</Link></h2><p>{articles[0].dek}</p><Link className="read-link" href={`/articles/${articles[0].slug}`}>Read the essay <span>↗</span></Link></div><Link className="feature-photo" href={`/articles/${articles[0].slug}`} aria-label={`Read ${articles[0].title}`}><Image unoptimized src={articles[0].thumbnail ?? "/images/articles/i-knew-the-factories-i-had-not-yet-seen-guangdong/05-shenzhen-talent-park-skyline.webp"} alt={articles[0].thumbnailAlt ?? articles[0].title} fill sizes="(max-width: 800px) 100vw, 32vw" style={{objectFit:articles[0].slug === "the-smallest-risk-on-a-construction-project" ? "contain" : "cover",objectPosition:"center"}} /></Link></section>
