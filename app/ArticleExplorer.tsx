...
export default function ArticleExplorer() {
  ...
  return <section className="article-explorer" aria-label="Explore articles">
    ...
    <div className="explorer-summary"><p role="status" aria-live="polite">{results.length} {results.length === 1 ? "essay" : "essays"}{query.trim() ? ` matching “${query.trim()}”` : " to explore"}</p>{(query || topic !== "all" || order !== "newest") && <button className="reset-filters" onClick={reset}>Reset filters</button>}</div>
    {results.length ? <div className="archive">{results.map(article => <ArticleCard key={article.slug} article={article} showThumbnail />)}</div> : <div className="explorer-empty"><h2>No essays found.</h2><p>Try a broader phrase or choose another topic.</p><button className="button" onClick={reset}>Show all essays</button></div>}
  </section>;
}
